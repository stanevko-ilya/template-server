...
function swaggerAuthMiddleware(req: Request, res: Response, next: NextFunction): void {
    ...
    if (!provided || provided !== serviceKey) {
        res.status(403).send('Доступ запрещён. Используйте /api/docs?key=YOUR_SERVICE_KEY');
        return;
    }
    next();
}
