...
import { processBatches, groupByNameAndCount, type BatchGroup } from '../../src/functions/batchPipeline.js';
import RateLimiter from '../../src/functions/rateLimiter.js';

function makeGroups(userIds: number[]): Map<string, BatchGroup> {
...
describe('batchPipeline.processBatches', () => {
    ...
    it('не ретраит ошибки с noRetry и считает их как errors', async () => {
        ...
        const res = await processBatches({
            ...
            sendBatch: async () => {
                attempts++;
                const e = new Error('auth') as Error & { noRetry?: boolean };
                e.noRetry = true;
                throw e;
            },
            rateLimiter: new RateLimiter(100000, 100),
