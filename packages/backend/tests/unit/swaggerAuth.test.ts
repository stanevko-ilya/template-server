...
import { swaggerAuthMiddleware } from '../../src/modules/api/swagger.js';
...
function makeSession(serviceKey: string, exp: number): string {
    const sig = createHmac('sha256', JWT).update(`swagger-ui:${serviceKey}:${exp}`).digest('hex');
...
function extractSessionCookie(res: request.Response): string {
    ...
    return cookie.split(';')[0]; // "swagger_session=<token>"
}
...
describe('swaggerAuthMiddleware — гейт доступа к /api/docs', () => {
    ...
    it('корректно подписанная НЕистёкшая сессия → 200', async () => {
        const res = await request(app)
            .get('/api/docs/swagger-ui-init.js')
            .set('Cookie', makeSession(KEY, Date.now() + 60_000));
        expect(res.status).toBe(200);
    });
    ...
    it('РЕГРЕСС: истёкшая (но корректно подписанная) сессия → 403 (серверная проверка срока)', async () => {
        const res = await request(app)
            .get('/api/docs/swagger-ui-init.js')
            .set('Cookie', makeSession(KEY, Date.now() - 1_000));
        expect(res.status).toBe(403);
    });

    it('подделка срока (exp в значении ≠ exp в подписи) → 403', async () => {
        const exp = Date.now() + 60_000;
        const sig = createHmac('sha256', JWT).update(`swagger-ui:${KEY}:${exp - 1}`).digest('hex');
        const res = await request(app)
            .get('/api/docs/swagger-ui-init.js')
    ...
    it('/api/docs (без слэша) с ключом → 302 на /api/docs/ БЕЗ ключа в Location (ключ не утекает)', async () => {
        ...
        // cookie выставлена на этом же 302 — follow-up авторизуется ею, ключ в URL не нужен
        expect(extractSessionCookie(res)).toMatch(/^swagger_session=\d+\.[a-f0-9]{64}$/);
    });
