...
async function isMongoAvailable(): Promise<boolean> {
    try {
        const conn = await mongoose
            .createConnection(process.env.DB_URL as string, { serverSelectionTimeoutMS: 3000 })
            .asPromise();
        await conn.close();
        return true;
    } catch {
...
describe('Интеграционный тест модулей', () => {
    ...
    it('DB запускается и имеет статус on (если MongoDB доступна)', async (ctx) => {
        if (!mongoAvailable) ctx.skip();
