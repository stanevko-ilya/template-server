...
Метод `log` первым параметром принимает уровень сообщения: `'info'`, `'warn'`, `'error'` или `null`. Также заместа метода `log` можно использовать `info`,`warn` и `error`, которые автоматически подставляют соответсвующий префикс к сообщению.
