# Система логирования
Данный модуль позволяет логировать действия на сервере.
...
## Использование
Для логирования необходимо вызвать метод `log` у объекта `logger`:
```javascript
const { logger } = require('modules.js');
logger.log('info', 'тест');
```

Метод `log` первым параметром принимает уровень сообщения: 'info', 'warn', 'error' или null. Также вместо метода log можно использовать `info`, `warn` и `error`, которые автоматически подставляют соответствующий префикс к сообщению.
